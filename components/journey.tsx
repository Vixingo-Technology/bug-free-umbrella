...
export default function Journey() {
  const belts = [
    { level: "10th - 9th Kyu", color: "white", bg: "bg-white", border: "border-zinc-300" },
    { level: "8th Kyu", color: "yellow", bg: "bg-yellow-400", border: "border-yellow-500" },
    { level: "7th Kyu", color: "orange", bg: "bg-orange-500", border: "border-orange-600" },
    { level: "6th Kyu", color: "green", bg: "bg-green-600", border: "border-green-700" },
    { level: "5th - 4th Kyu", color: "blue", bg: "bg-blue-600", border: "border-blue-700" },
    { level: "3rd - 1st Kyu", color: "brown", bg: "bg-amber-800", border: "border-amber-900" },
    { level: "Dan (Black)", color: "black", bg: "bg-zinc-950", border: "border-zinc-800" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="hidden lg:flex items-center justify-between relative mt-32 mb-20">
          ...
          {belts.map((belt, i) => (
            ...
            >
              ...
              <div className={`w-16 h-16 rounded-full border-[4px] ${belt.border} ${belt.bg} shadow-md flex items-center justify-center transition-transform duration-500 group-hover:scale-125`}>
                {belt.color === 'black' && <div className="w-8 h-[2px] bg-accent-red"></div>}
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="lg:hidden flex flex-col gap-8 mt-12 relative border-l border-zinc-200 pl-6 ml-4">
          {belts.map((belt, i) => (
            ...
            >
              <div className={`absolute -left-[41px] w-8 h-8 rounded-full border-2 ${belt.border} ${belt.bg} shadow-sm`}></div>
              <div>
                <h4 className="text-zinc-800 font-heading tracking-widest uppercase text-sm mb-1 font-bold">{belt.color} Belt</h4>
                <p className="text-zinc-600 text-xs font-mono">{belt.level}</p>
              </div>
            </motion.div>
          ))}
        </div>
