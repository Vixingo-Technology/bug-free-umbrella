            ...
            >
              ...
              <div className="flex-1 mb-6 md:mb-0">
                <div className="flex items-center gap-4 mb-4">
                  <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 border rounded-sm ${evt.status === 'Completed' ? 'border-zinc-200 text-zinc-400 bg-zinc-50' : 'border-accent-red/20 text-accent-red bg-accent-red/5'}`}>
                    {evt.status}
                  </span>
                  <span className="text-xs uppercase tracking-widest text-zinc-500 font-semibold">{evt.type}</span>
                </div>
                <h3 className={`text-2xl font-heading font-bold tracking-wide mb-4 ${evt.status === 'Completed' ? 'text-zinc-400 line-through' : 'text-zinc-900'}`}>
                  {evt.title}
                </h3>
              ...
              <div>
                ...
                >
                  {evt.status === 'Completed' ? 'View Results' : 'Register Now'}
                </button>
              </div>
            </motion.div>
          ))}
