...
function VerifyContent() {
    ...
    function handleSubmit() {
        ...
        startTransition(async () => {
            const result = await verifySignupOtp(email, joined);
            if (result?.error) {
            ...
            <div className="w-full max-w-md mx-auto px-6 relative z-10">
                ...
                >
                    <div className="flex flex-col items-center gap-4 mb-8">
                        <Image src={Logo} alt="JKA Bangladesh logo" width={56} height={56} />
                    ...
                    <div className="flex justify-center gap-2 mb-6">
                        {code.map((d, i) => (
                            <input
                                ...
                                onPaste={handlePaste}
                                disabled={isPending}
                                className="w-12 h-14 md:w-14 md:h-16 text-center font-karate text-2xl font-bold text-zinc-900 bg-white border border-zinc-300 focus:outline-none focus:border-accent-red rounded-sm transition-colors disabled:opacity-60"
                            />
                        ))}
                    </div>
